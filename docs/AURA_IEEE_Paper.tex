...
\author{
  \IEEEauthorblockN{Eric T. K.\IEEEauthorrefmark{1}, Navaneet Krishnan R. V.\IEEEauthorrefmark{1}, Pranav R. Mallia\IEEEauthorrefmark{1}}
...
46.37\% better API P99 latency (23.13\,ms vs 43.13\,ms baseline) while
...
\begin{enumerate}
  ...
  \item \textbf{Real-cluster evaluation}: Experimental results on live
      k3d cluster demonstrating 46.37\% better API P99 latency (23.13\,ms
...
\begin{equation}
\begin{aligned}
s_t^i = [\,&\mathrm{cpu}_i,\;\mathrm{mem}_i,\;\mathrm{lat}^{p50}_i,\;\mathrm{lat}^{p95}_i,\;\mathrm{lat}^{p99}_i,\;\mathrm{rps}_i,\\
&\mathrm{err}_i,\;\mathrm{queue}_i,\;\Delta\mathrm{rps}_i,\;n_i^{\mathrm{desired}},\;n_i^{\mathrm{ready}},\;r_i^{\mathrm{ratio}},\\
&\mathrm{cpu}_{i,t-1},\;\Delta\mathrm{cpu}_i,\;p_i^{\mathrm{downstream}},\;\mathrm{lat}^{p95}_i\,]
\end{aligned}
\label{eq:state}
\end{equation}
...
\begin{equation}
  a_t^i \in \mathcal{A}^i = \{-5,\ldots,+5\}
  \label{eq:action}
\end{equation}
...
\mathcal{L}(\theta,\phi) =
\mathbb{E}\Big[\big(
&r_t + \gamma\max_{\mathbf{a}'} Q_{\mathrm{tot}}^{\text{target}}(s_{t+1}, \mathbf{a}') \\
&- Q_{\mathrm{tot}}(s_t, \mathbf{a}_t)
\big)^2\Big]
\end{aligned}
\label{eq:loss}
...
The live controller (\path{deployment/agent_controller.py}) is
...
\textbf{API Tier Success:} QMIX achieves \textbf{46.37\% lower API P99 latency}
...
HPA, QMIX achieves 4.32$\times$ better API P99 (23.13\,ms vs.\ 99.87\,ms)
...
HPA scaled APP to an average of 2.59 replicas. Post-hoc controller
inspection shows this was primarily a control-path bug: APP scale-up
actions were suppressed by tier-coupled guard logic under API bottleneck
conditions, after which min-replica clamping pinned APP at 1. In the
recorded APP decision stream (61 decisions) from
\texttt{qmix\_decisions\_20260218\_171840.csv}, +1 never occurred;
40 decisions were -1 and 21 were 0.

\textbf{Throughput Trade-off:} HPA achieves 3.58$\times$ higher throughput
...
and 20.95\% error rate are explained by controller guard behavior rather
than a proven QMIX policy failure. Specifically, APP scale-up was blocked
by tier-coupled veto logic in the controller and then bounded by
$\text{MIN\_REPLICAS}=1$, which kept APP at one replica. This bug masks
the true APP-tier capability of the learned policy and should be fixed
before drawing architecture-level conclusions.
...
\end{axis}
\end{tikzpicture}
\caption{Training convergence over 2\,000 episodes:
QMIX reaches $-73.2$ vs. IDQN $-119.2$.}
...
However, the APP under-scaling observed in this run cannot be attributed
to reward weights alone because controller veto/clamp logic altered
actions before execution. Therefore, clean ablation on
$(\alpha,\beta,\gamma)$ requires re-running experiments after removing the
APP scale-up suppression bug. The churn penalty $\gamma = 1.5$ still
demonstrably prevents oscillatory
...
\subsection{QMIX Action Profile}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/qmix_action_profile.pdf}
\caption{Distribution of QMIX scaling actions ($-1$, $0$, $+1$) per service
during the production run. API receives most positive scaling actions,
while APP and DB remain mostly unchanged, matching the observed tier-aware behavior.}
\label{fig:qmix-action-profile}
\end{figure}

Fig.~\ref{fig:qmix-action-profile} summarizes controller behavior from
real decision logs and shows that QMIX concentrates scale-up actions on
the API tier while applying mostly hold/scale-down decisions on APP and DB.
...
\path{infra/manifests/three-tier/} for \path{envoy-config-*.yaml})
exposes:

\begin{itemize}
\item \textbf{HTTP metrics:} \path{envoy_http_downstream_rq_total}
  (RPS), \path{envoy_http_downstream_rq_time_bucket} (latency
  histograms), \path{envoy_http_downstream_rq_xx} (status codes)
\item \textbf{Queue depth:} \path{envoy_http_downstream_rq_active}
...
resources (see \path{*-servicemonitor.yaml} in the same directory).
...
The AURA controller (see \path{deployment/agent_controller.py})
...
demonstrated that QMIX achieves \textbf{46.37\% better API P99 latency}
(23.13\,ms vs.\ 43.13\,ms baseline) while using 50\% more resources,
and \textbf{4.32$\times$ better API latency than HPA} (23.13\,ms vs.\
99.87\,ms) while using 55\% less CPU (0.90 vs.\ 2.00 cores).

The results also revealed a controller-level bug: APP scale-up actions
were suppressed by tier-coupled guard logic and then clamped by
$\text{MIN\_REPLICAS}=1$, leaving APP fixed at one replica and causing
elevated APP latency/error (P99 780.81\,ms, 20.95\%). This is an
execution-path issue and not sufficient evidence of a QMIX architecture
limitation.

Future directions include: (1)~removing APP scale-up suppression in the
controller and re-running full evaluations, (2)~extending AURA to
